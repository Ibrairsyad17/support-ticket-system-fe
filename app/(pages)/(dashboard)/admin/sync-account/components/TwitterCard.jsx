...
import { Link1Icon, Cross1Icon, ExitIcon } from "@radix-ui/react-icons";
  ...
  return (
    <div className="grid grid-cols-1 gap-3 shadow-md shadow-gray-100 rounded-lg py-5 px-4">
      <div className="flex w-full justify-between items-center">
        ...
        >
          <X className={`w-4 h-4 flex-shrink-0 size-4`} sx={{ fontSize: 20 }} />
        </div>
      ...
      {data.status === "DISCONNECTED" && (
        <Button asChild className="text-xs" size="sm">
          <Link href={`/sync-account/x`}>
            <Link1Icon className="mr-2 h-3 w-3" /> Hubungkan Twitter
          </Link>
        </Button>
      )}
    </div>
